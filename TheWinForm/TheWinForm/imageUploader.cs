...
namespace TheWinForm
{
    ...
    public class imageUploader
    {
        ...
        public static void UploadImage(string username, byte[] image)
        {
            ...
            using (SqlConnection sqlCon = new SqlConnection(@"Data Source=LAB109PC14\SQLEXPRESS; Initial Catalog=SKibidiBap; Integrated Security=True;"))
            {
                sqlCon.Open();  // Opening the database connection

                string uploadImgQ = "UPDATE Table_1 SET img=@image WHERE username=@username";  // SQL query to update the image for a specific username
